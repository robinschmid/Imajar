...
public class ContourPlotJFreeChart extends EImage2DChartPanel {
  ...
  public static ContourPlotJFreeChart createChart(Image2D img, int interpolationFactor,
      double isoFactor) {
    ...
    double[][] interpolatedData = new double[data.length * interpolationFactor
        - correction][data[0].length * interpolationFactor - correction];
    ...
    // * For each isoColor draw polygon in the specific isoCell if ternary index is different than 0
    // and 80.
    // */
    ...
    // if (contains) {
    // average /= 4;
    // int ternaryIndexOfAverageOfCorners = checkIfValueIsInRange(startOfRange, endOfRange,
    // average);
