...
public class ImageRenderer extends AbstractXYItemRenderer
  ...
  protected void drawBlockItem(Graphics2D g2, XYItemRendererState state, Rectangle2D dataArea,
    ...
    double xx1 = domainAxis.valueToJava2D(x + this.getBlockWidth(series) + this.xOffset, dataArea,
        plot.getDomainAxisEdge());
    double yy1 = rangeAxis.valueToJava2D(y + this.getBlockHeight(series) + this.yOffset, dataArea,
    ...
    if (dataArea.intersects(block)) {
      ...
      if (isItemLabelVisible(series, item)) {
        drawItemLabel(g2, orientation, dataset, series, item, block.getCenterX(),
            block.getCenterY(), y < 0.0);
      }
