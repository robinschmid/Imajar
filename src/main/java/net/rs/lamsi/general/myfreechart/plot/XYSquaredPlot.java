 ...
 *
 * An extension to the XYPlot provided in JFreeChart, this class draws XY plots where the spacing
 * between two given values is the same on both the X and Y axis. This class over-rides draw() in
 * XYPlot to add it's functionality.
 */
...
/**
 * This plot keeps one axis at static range and scales the opposite range in a way that both axis
 * have the same value-to-java2d dimensions. A box from 0;0 to 1;1 will be a perfect square. For
 * graphics export: Supply a ChartRenderingInfo to the JFreeChart.draw method
 * 
 * @author Robin Schmid
 *
 */
public class XYSquaredPlot extends XYPlot {
  ...
  /*
   * public SquaredXYPlot(XYDataset dataset, ValueAxis domainAxis, ValueAxis rangeAxis,
   * XYItemRenderer renderer)
   */
  public XYSquaredPlot(XYDataset dataset, ValueAxis domainAxis, ValueAxis rangeAxis,
      XYItemRenderer renderer) {
    this(dataset, domainAxis, rangeAxis, renderer, Scale.DYNAMIC);
  }

  public XYSquaredPlot(XYDataset dataset, ValueAxis domainAxis, ValueAxis rangeAxis,
      XYItemRenderer renderer, Scale smode) {
    super(dataset, domainAxis, rangeAxis, renderer);
    this.smode = smode;
  }

  /**
   * Calc new chartarea returns the same chartarea if the plot is square returns a new chartarea
   * object if not
   * 
   * @param g2
   * @param chartarea
   ...
   * @return
   */
  public Rectangle2D calc(Graphics2D g2, Rectangle2D chartarea, Rectangle2D plotarea,
      PlotRenderingInfo info, int iteration) {
  ...
  private void printInfo(PlotRenderingInfo info) {
    ...
    if (info.getOwner() != null)
      logger.debug("Chart area {}", info.getOwner().getChartArea().toString());
    if (info.getPlotArea() != null)
      logger.debug("Plot area {}", info.getPlotArea().toString());
    if (info.getDataArea() != null)
      logger.debug("Data area {}", info.getDataArea().toString());
  }
