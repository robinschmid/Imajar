...
public class JMultiRangeSlider extends JPanel implements MouseMotionListener, MouseListener {
  ...
  public static final Color[] MARKERS = new Color[] {Color.MAGENTA, Color.BLUE, Color.CYAN,
      Color.GREEN, Color.YELLOW, Color.ORANGE, Color.RED};
  // with steps in between
  public static final Color[] MARKERS_HALFSTEPS =
      new Color[] {Color.MAGENTA, new Color(127, 0, 255), Color.BLUE, new Color(0, 127, 255),
          Color.CYAN, new Color(0, 255, 127), Color.GREEN, new Color(127, 255, 0), Color.YELLOW,
   ...
   * Create the panel.
   */
  public JMultiRangeSlider(boolean showValues, boolean useHalfSteps) {
      ...
      public void paint(Graphics g) {
        if (cStart != null) {
          ...
          for (int x = 1; x < w - 1; x++) {
            float p = (x - 1.f) / (w - 2.f);
            Color c =
                PaintScaleGenerator.interpolateWeighted(cStart, cEnd, p, hue, position, false);
            g.setColor(c);
  ...
  /**
   * hue and positions are reset try using setup if positions and hue values already exist
   * 
   * @param cStart
   * @param cEnd
   */
  public void setColors(Color cStart, Color cEnd) {
    ...
    for (int i = 0; i < size; i++) {
      if (inverted) {
        ...
        if (hStart <= getHue(mark[size - 1 - i]) && startMarker == -1)
          startMarker = size - i;
        if (hEnd >= getHue(mark[i]) && endMarker == -1)
          endMarker = i;
      }
    }
    ...
    if (endMarker != -1 && Math.abs(startMarker - endMarker) > 0) {
      ...
      for (int i = 0; i < hue.length; i++) {
        if (inverted)
          hue[i] = getHue(mark[startMarker - i]);
        else
          hue[i] = getHue(mark[startMarker + i]);

        position[i] = getPosition(hStart, hEnd, hue[i]);
        if (!inverted)
          position[i] = 1.f - position[i];
      }
    }
    selectedPicker = -1;
    revalidate();
  ...
  private void setPositionOf(int i, float p) {
    ...
    if (p < 0)
      p = 0;
    if (i - 1 >= 0 && position[i - 1] > p)
      p = position[i - 1];
    if (p > 1.f)
      p = 1.f;
    if (i + 1 < position.length && position[i + 1] < p)
      p = position[i + 1];
