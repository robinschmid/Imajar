...
public class DataInterpolator {
  ...
  /**
   * interpolates (f>1) / or reduces (f<0) the data by the factor f reduction is only performed for
   * data points in a line interpolation is performed in both dimensions
   * 
   * @param data
   * @param f
   * @return
      ...
      for (int i = 0; i < z.length * interpolationFactor - correction; i++) {
        ...
        for (int j = 0; j < data.lineLength((int) idx) * interpolationFactor - correction; j++) {
          ...
          if (!Double.isNaN(value)) {
            ...
            double yh = idx + 1 < z.length ? y[(int) idx + 1][(int) jdy] - y[(int) idx][(int) jdy]
