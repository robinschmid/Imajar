...
public class Simple2DDataset extends ImageDataset {
	...
	public float getX(int line, int dpi) {
		return intensity!=null || intensity.length<line || intensity[line].length<dpi? dpi*pixelWidth : 0;
	}

	@Override
	public double getI(int index, int line, int dpi) {
		return intensity!=null || intensity.length<line || intensity[line].length<dpi? intensity[line][dpi]  : 0;
	}
	...
	public int getTotalDPCount() {
		if(totalDPCount!=-1) return totalDPCount;
		else {
			if(intensity!=null) {
				totalDPCount = 0;
	...
	@Override
	public int getMaxDP() {
		if(longestLineDPCount!=-1) return longestLineDPCount;
		else {
			if(intensity!=null) {
				longestLineDPCount = 0;
				for(double[] i : intensity)
					if(longestLineDPCount<i.length) longestLineDPCount = i.length;
	...
	@Override
	public int getMinDP() {
		if(shortestLineDPCount!=-1) return shortestLineDPCount;
		else {
			if(intensity!=null) {
				shortestLineDPCount = 0;
				for(double[] i : intensity)
					if(shortestLineDPCount>i.length) shortestLineDPCount = i.length;
	...
	public float getRightEdgeX(int line) {
		return intensity!=null || intensity.length<line? pixelWidth * (intensity[line].length+1) : 0 ;
	}
