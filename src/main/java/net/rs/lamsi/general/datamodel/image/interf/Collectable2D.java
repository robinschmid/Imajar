...
public abstract class Collectable2D<T extends SettingsContainerSettings> implements Serializable {
  ...
  /**
   * image is marked as a group member in an image group. This group handles multi dimensional data
   * sets (not only)
   * 
   * @param imageGroupMD
   */
  ...
  public boolean isSPImage() {
    return SingleParticleImage.class.isInstance(this);
  }
  ...
  /**
   * try to set a new project name checks if this name is not empty and unique
   * 
   * @param name new name
  ...
  public String setProjectName(String name) {
    ...
    if (tree.getProject(name) == null) {
      ...
      return getImageGroup().getProject().getName();
  }
  ...
  public String setGroupName(String name) {
    if (getImageGroup() == null)
      ...
      return getImageGroup().setGroupName(name);
  }
}
