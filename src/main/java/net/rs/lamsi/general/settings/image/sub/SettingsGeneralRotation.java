...
public class SettingsGeneralRotation extends Settings {
  ...
  public void setAll(IMAGING_MODE imagingMode, boolean reflectHoriz, boolean reflectVert,
      int rotationOfData) {
  ...
  public void loadValuesFromXML(Element el, Document doc) {
    ...
    for (int i = 0; i < list.getLength(); i++) {
      if (list.item(i).getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (paramName.equals("imagingMode"))
          imagingMode = IMAGING_MODE.valueOf(nextElement.getTextContent());
        else if (paramName.equals("rotationOfData"))
          rotationOfData = intFromXML(nextElement);
        else if (paramName.equals("reflectHorizontal"))
          reflectHorizontal = booleanFromXML(nextElement);
        else if (paramName.equals("reflectVertical"))
          reflectVertical = booleanFromXML(nextElement);
      }
    }
  }
  ...
  public boolean isRotated() {
    return !(rotationOfData == 0 || rotationOfData == 180 || rotationOfData == 360);
  }
}
