...
public class Quantifier implements Serializable, IntensityProcessingChangedListener {
  ...
  // choose region
  protected boolean useStart, useMiddle, useEnd, isActive = true, useSelectedExcluded = true,
      useRawData = false;
  ...
  /**
   * for mode average all: uses all dp or average boxes: uses selection and exclusion model
   */
  ...
  /**
   * call for getting the average intensity of a line if mode is average it will return the average
   * over all lines!
   * 
   * @param line
   * @return
  ...
  public double getAverageIntensity(int line) {
    ...
    if (mode == MODE_AVERAGE_BOXES && (averageI == -1 || averageI == Double.NaN))
      calcAverageIRects();
    else if (mode != MODE_AVERAGE_BOXES
        && (averageILines == null || (averageI == -1 || averageI == Double.NaN)))
