...
public class SettingsImageGroup extends SettingsContainerSettings implements GroupSettings {
    ...
    for (int i = 0; i < list.getLength(); i++) {
      if (list.item(i).getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (paramName.equals("name"))
          name = nextElement.getTextContent();
        else if (paramName.equals("pathData"))
          pathData = nextElement.getTextContent();
      }
    }
  ...
  public void setName(String name) {
    this.name = FileAndPathUtil.replaceInvalidChar(name, "_");
  }
