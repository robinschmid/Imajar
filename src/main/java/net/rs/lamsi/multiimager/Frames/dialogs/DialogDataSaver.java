...
public class DialogDataSaver extends JFrame {
  ...
  public DialogDataSaver(SettingsHolder sett) {
    ...
    contentPane.setLayout(new MigLayout("", "[56px,grow][6px][100px][6px][109px,grow][9px]",
        "[14px][23px][23px][23px][243px,center][33px]"));
    ...
    cbSaveAllFilesInOne
        .setToolTipText("Save data from all files in one xls-file or in seperate ones");
    ...
    comboDataFormat.setToolTipText(
        "Select the data export pattern: X-matrix is the standard and exports one xmatrix.txt file to keep information about data point width. XYYY is for equally spaced data points. XYXY (altern.) is for data points not spaced equally. Only-Y has no information about spacing.");
    ...
    cbArray.setToolTipText(
        "Save all data points that are excluded, selected or selected and not excluded to data columns.");
    ...
    cbShapesSelNEx.setToolTipText(
        "Export all selection shapes in regards to exclusions (cut out data matrix)");
    ...
    cbShapes.setToolTipText(
        "Export all shapes with no regards to exclusions. All data points inside the shape are exported.");
    pnExportSelections.add(cbShapes, "cell 0 7");
    cbShapes.setSelected(true);

    cbShapesData = new JCheckBox("Shape data summary");
    cbShapesData
        .setToolTipText("Writes all data points that were used for statistics for each shape");
  ...
  protected void exportData(SettingsImage2DDataExport s) {
    ...
    try {
      ...
      if (currentMode == MODE.ALL) {
        ...
        DataExportUtil.exportDataImage2DInRects(img, selections,
            settings.getSetImage2DDataSelectionsExport());
      }
    } catch (Exception ex) {
  ...
  private SettingsImage2DDataExport renewAllSettings() {
    if (currentMode.equals(MODE.ALL)) {
      ...
      settingsData
          .setExportStructure((EXPORT_STRUCTURE) getComboExportStructure().getSelectedItem());
      ...
      s.setAll(cbSummary.isSelected(), cbDef.isSelected(), cbArray.isSelected(),
          cbImgEx.isSelected(), cbImgSel.isSelected(), cbImgSelNEx.isSelected(),
          cbShapes.isSelected(), cbShapesSelNEx.isSelected(), cbShapesData.isSelected(),
