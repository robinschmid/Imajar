...
public class ShapeSelectionsTableModel extends AbstractTableModel {
  ...
  public int getRowCount() {
    return selections == null || selections.getSelections() == null ? 0
        : selections.getSelections().size();
  }
    ...
    switch (col) {
      ...
      case 1:
        ...
        if (!sel.getMode().equals(nm)) {
          ...
          boolean update =
              sel.getMode().equals(SelectionMode.EXCLUDE) || nm.equals(SelectionMode.EXCLUDE);
          // set mode
          sel.setMode(nm);
          if (update)
            selections.updateStatistics();
  ...
  public Object[][] toArray(boolean useTitle) {
    ...
    try {
      ...
      for (int i = 0; i < getRowCount(); i++)
        data[i + 1] =
            selections.getSelections().get(i).getRowData(selections.isAlphaMapExclusionActive());
