...
public class CroppingDialog extends JFrame {
      ...
      }
      {
        ...
        btnNext.addActionListener(new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            ...
            selectedImgIndex =
                selectedImgIndex < c - 1 ? selectedImgIndex + 1 : group.image2dCount() - 1;
            setSelectedImageAndShow((Image2D) group.get(selectedImgIndex, Image2D.class));
          }
        });
          ...
          });
          {
            ...
            }
            {
              ...
              panel_2.add(btnAuto, "cell 0 1");
              {
                ...
                txtAllowHighs.setToolTipText(
                    "Allowance for X consecutive high signal (e.g. noise) in the low background");
              ...
              }
              {
                ...
                txtMinimum.setToolTipText(
                    "New background level for auto detection (the paint scale minimum is used if no value is set)");
  ...
  /**
   * auto adjust by searching for an intensity drop in each line (low->high) where low is defined by
   * <minimum of paint scale allow some consecutive high signals
   */
  ...
  protected void shiftLineStart(int shift) {
    ...
    for (int i = 0; i < settings.getShiftLineStart().length; i++)
      if (settings.getShiftLineStart()[i] < min)
        min = settings.getShiftLineStart()[i];
    for (int i = 0; i < settings.getShiftLineStart().length; i++)
  ...
  public void startDialog(ImageGroupMD group, Image2D selectedImage) {
    ...
    getLbLastLineToShort().setVisible(selectedImage.getData()
        .getLineLength(selectedImage.getData().getLinesCount() - 1) < avg * 0.95);
    // update heatmap
