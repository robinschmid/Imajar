...
/**
 * this one is for direct imaging analysis it runs in a loop
 * loading the files every T min and updating the images
 * @author vukmir69
 *
 */
public class DirectImageLogicRunner implements Runnable {
	...
	*/
	...
	/* set up new tasks
	 * - add images to tree
	 * - all finished lines and filedim to diatask
	 * next run: search for new files - compare with old dim
	 * - new task found?
	 * - - new task = more File[] in sub vector
	 */
	@Override
	public void run() {
...
//		}
	...
	// run in a loop
		...
		public void run() {
			...
			while(!isPaused()) { 
				long time1 = System.currentTimeMillis();
				// files will be stored in files
				File[] files = findFilesInDir(dir);
				
				Vector<File[]> sub = FileAndPathUtil.findFilesInDir(dir, sett.getFilter(), true, sett.isFilesInSeparateFolders());
				
				for(File[] i : sub) {
					// load them as image set
					Image2D[] imgs = Image2DImportExportUtil.importTextDataToImage(i, sett, true); 
					ImageEditorWindow.log("Imported image "+i[0].getName(), LOG.DEBUG);
					if(imgs.length>0) {
						// add img to list
						addCollection2D(imgs, fnode, sett);
					}
				}
				...
				if(files!=null && files.length>0) {
					...
					if(lastFiles==null) {
						...
						Image2D[] imgList;
						try {
							imgList = Image2DImportExportUtil.importTextDataToImage(files, sett, false);
							// save lines here in lines vector
							lines = new Vector[imgList.length];
							//
							IconNode parent = new IconNode(dir.getName()+"; "+dir.getAbsolutePath()); 
							runner.addCollection2D(imgList, parent, sett);
							runner.getTree().addNodeToRoot(parent);
							// add each image and add all line of images to lines vector
							for(int img=0; img<imgList.length; img++) { 
								// add all lines
								lines[img] = imgList[img].getLinesAsVector();
							}
							if(imgList.length>0)
								lastFiles = dim;
						} catch (Exception e) { 
						...
						try {
							...
							if(newFiles.length>0) {
								// get current Image2D Vector
								Vector<Image2D> imgList = runner.getListImages();
								// create new Vector<ScanLine> [] (kind of Image2D without settings)
								...
								// same size?
								if(newLines!=null && imgList.size()==newLines.length) {
									...
									for(int i=0; i<imgList.size(); i++) {
										...
										imgList.get(i).setLines(lines[i].toArray(new ScanLine[lines[i].size()]));
									} 
									// send update to 
									runner.renewImage2DView();
								}
									...
									lastFiles = null;
									continue; // next while loop to load images new
								}
								
								//check all 10 times if there was an error and create images new if needed  
								// new version to find new files
	 ...
	 * @param sumTasks 
	 */
	public void setUp(long sleep, String fileFilterExt, String startsWith, boolean sumTasks, boolean autoScale, double scaleFactor) {
		this.sleepSec = sleep;
