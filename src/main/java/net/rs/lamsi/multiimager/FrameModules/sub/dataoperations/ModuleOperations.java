...
public class ModuleOperations
    extends Collectable2DSettingsModule<SettingsImage2DOperations, Image2D> {
  ...
  public ModuleOperations(ImageEditorWindow wnd) {
    ...
    btnChooseFromList.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        try {
          int i = DialogLoggerUtil.showListDialogAndChoose(window, list,
    ...
    cbBlankBothSides
        .setToolTipText("Uses the data at the start and at the end to do blank reduction");
  ...
  /**
   * sets the IS and checks for blank in both images same blank for IS and sample
   * 
   * @param img
   */
  ...
  protected void setBlank(Image2D img) {
    if (img == null) {
      ...
      if (getRbBlankActualDataPoint().isSelected()
          && (img.getLineCount(0) != currentImage.getLineCount(0)
              || img.getData().getAvgDP() != currentImage.getData().getAvgDP()))
  ...
  private void setAllViaExistingSettingsBlank(SettingsImage2DBlankSubtraction q) {
    ...
    getRbBlankActualDataPoint()
        .setSelected(q.getModeAvg() == SettingsImage2DBlankSubtraction.MODE_ACTUAL_DP);
    getRbBlankAverage().setSelected(q.getModeAvg() == SettingsImage2DBlankSubtraction.MODE_AVERAGE);
    getRbBlankAveragePerLine()
        .setSelected(q.getModeAvg() == SettingsImage2DBlankSubtraction.MODE_AVERAGE_PER_LINE);
    // set tab
    getTabbedBlank()
        .setSelectedIndex(q.getModeData() == SettingsImage2DBlankSubtraction.MODE_DATA_IMG ? 0 : 1);
    // set tab img
    if (imgBlank == null) {
      getTxtBlankPath().setText("");
      getTxtBlankTitle().setText("");
  ...
  public SettingsImage2DOperations writeAllToSettings(SettingsImage2DOperations sett) {
    if (sett != null) {
      try {
        ...
        int mode = getRbBlankActualDataPoint().isSelected()
            ? SettingsImage2DBlankSubtraction.MODE_ACTUAL_DP
            : SettingsImage2DBlankSubtraction.MODE_AVERAGE;
        if (getRbBlankAveragePerLine().isSelected())
          mode = SettingsImage2DBlankSubtraction.MODE_AVERAGE_PER_LINE;
        sett.getBlankQuantifier().setMode(mode);
        // same image starting: tab
        sett.getBlankQuantifier().setModeData(
            tabbedBlank.getSelectedIndex() == 0 ? SettingsImage2DBlankSubtraction.MODE_DATA_IMG
                : SettingsImage2DBlankSubtraction.MODE_DATA_START);
