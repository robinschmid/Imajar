...
public abstract class PnTableQuantifier extends JPanel implements ListSelectionListener {
  ...
  protected String[] columnToolTips = {"Concentration", "Name of image", "Path of raw file",
      "Name of parent. Parent has master settings.", "Average calculation with all data points",
      "Average calculation with selection boxes", "Select data"};
  ...
  public PnTableQuantifier(ImageEditorWindow wnd) {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        WindowAdapter wl = new WindowAdapter() {
          ...
          public void windowClosed(WindowEvent e) {
            ...
            getTableModel().fireTableCellUpdated(modelRow,
                QuantifierTableColumnType.SELECT.getIndex());
          }
        };
    ...
    ButtonColumn buttonColumn = new ButtonColumn(tableQuantifier, openSelectBoxDialog,
        QuantifierTableColumnType.SELECT.getIndex());
    ...
    Action openSelectParentDialog = new AbstractAction() {
      public void actionPerformed(ActionEvent e) {
        ...
        try {
          int parenti = row.getImg().getParent() == null ? 0
              : window.getLogicRunner().getListImages().indexOf(row.getImg().getParent());
          if (parenti == -1)
            parenti = 0;
          int i = DialogLoggerUtil.showListDialogAndChoose(window, list,
              ListSelectionModel.SINGLE_SELECTION, parenti)[0];
          // set parent and update
          row.getImg().setParent(i >= 0 && i < list.length ? (Image2D) list[i] : null);

          getTableModel().fireTableCellUpdated(modelRow,
              QuantifierTableColumnType.PARENT.getIndex());
        } catch (Exception ex) {
        }
      }
    };

    buttonColumn = new ButtonColumn(tableQuantifier, openSelectParentDialog,
        QuantifierTableColumnType.PARENT.getIndex());
    buttonColumn.setMnemonic(KeyEvent.VK_E);

    // set column mode to combobox
    TableColumn colMode =
        getTable().getColumnModel().getColumn(QuantifierTableColumnType.MODE.getIndex());
