...
public class ImageGeneratorRunner implements Runnable {
  ...
  public void run() {
    ...
    while (isRunning) {
      ...
      window.getLbLabel()
          .setText("10Images #  task=" + (task + 1) + " #  line=" + cl + " #  dp=" + cdp);
      //
  ...
  private void startNewFile(TxtWriter writer, String dir, String subdir, String file) {
    ...
    if (inFolder)
      ...
      writer.openNewFileOutput(dir + subdir + "/" + file + ".txt");
    ...
    if (inFolder)
      ...
      writer.openNewFileOutput(dir + subdir + "/" + file + ".csv");
    writer.writeLine("Header Stuff,dont try to read this,really!");
    writer.writeLine("Header Stuff,dont try to read this,really!");
    writer.writeLine("More Header Stuff");
    // write title
   ...
   */
  /*
   * public Image2D createImage() { ScanLine2D[] lines = new ScanLine2D[100]; for(int l=0;
   * l<lines.length; l++) { DataPoint2D[] dp = new DataPoint2D[1000]; for(int d=0; d<dp.length; d++)
   * { // middle the highest double in = (50-Math.pow(Math.abs(l+1-lines.length/2),2))*10+
   * 500-Math.pow(Math.abs(d+1-dp.length/2),2)*1; in += Math.abs(l+1-lines.length/2)*1 +
   * Math.abs(d+1-dp.length/2)*400; in += (lines.length-l)*5000; in +=
   * Math.abs(rand.nextInt(6000)/100.0); in = in/20; // create dp dp[d] = new DataPoint2D(d*0.1,
   * in); } lines[l] = new ScanLine2D(dp); } return new Image2D(new Dataset2D(lines)); } /**
   * creating standards 0 - 800
   */
  /*
   * public Image2D createStandards() { ScanLine2D[] lines = new ScanLine2D[24]; for(int l=0;
   * l<lines.length; l++) { DataPoint2D[] dp = new DataPoint2D[1000]; for(int d=0; d<dp.length; d++)
   * { // middle the highest double in = (int)(l/4)*200.0; in += Math.abs(rand.nextInt(6000)/100.0);
   * // create dp dp[d] = new DataPoint2D(d*0.1, in); } lines[l] = new ScanLine2D(dp); } return new
   * Image2D(new Dataset2D(lines)); }
   */
  ...
  public Image2D[] startImage(boolean inFolder) {
    ...
    /*
     * Image2D[] std = new Image2D[10]; for(int i=0; i<std.length; i++) std[i] = createStandards();
     * taskList.add(std);
     * 
     * Image2D[] image = new Image2D[10]; for(int i=0; i<image.length; i++) image[i] =
     * createImage(); taskList.add(image);
     * 
     * taskList.add(std);
     * 
     * taskList.add(image);
