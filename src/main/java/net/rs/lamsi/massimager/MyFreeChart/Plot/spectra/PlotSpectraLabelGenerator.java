package net.rs.lamsi.massimager.MyFreeChart.Plot.spectra;
...
public class PlotSpectraLabelGenerator implements XYItemLabelGenerator {
	...
	public String generateLabel(XYDataset dataset, int series, int item) {
		SettingsPlotSpectraLabelGenerator settings = Window.getWindow().getSettings().getSetVisPlotSpectraLabelGenerator();
		// no labels?
		...
		double yUpperBound = (double) plot.getChart().getXYPlot().getRangeAxis().getRange().getUpperBound();
	 ...
	 */
	private void getPossiblePeaksInDistance(Vector<Point2D> peaks, XYDataset dataset, int series, int item, int direction) {
