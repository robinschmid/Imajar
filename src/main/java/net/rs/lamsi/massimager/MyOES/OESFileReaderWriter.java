...
public class OESFileReaderWriter {
    ...
    public OESFile generateElementLineListFromTxtFile(File file) {
    	...
    	//System.out.println("geben sie den Dateipfad an, sonst gibt es ärger.D:/Programmieren/Ca-Werte ICP-OES2.txt");
    	try {
			...
			String kopfzeile="";//leerer string. muss vor for-schleife sein damit es nicht bei neuem schleifendurchgang gelöscht wird
	    			
	    	for(int i=1; i<liste.size(); i++) {
	    		String currentline=liste.get(i);//daten werden als zeichenkette eingelesen
	    		// Die falschen Lerrzeichen rauslöschen
	    		char leerzeichen = 0;
				currentline=currentline.replaceAll(String.valueOf(leerzeichen), "");//Leerzeichen in Schriftart in dokument ist hier eine null
				//
	    		if(currentline.length()>2){
	    			...
	    			//Daten finden und einlesen, leerzeichen rauslöschen
	    				...
	    				if (!currentline.startsWith("Element")){//wenn currentline nicht (=!) mit element anfängt, dann haben wir daten vorliegen
	    				    //   
	    					currentline=currentline.replaceAll(" ", "");//currentline.replacAll gibt string ohne leerzeichen zurück und speichert es als currentline (=überschreibung "altes" currentline)
	    					     //eckige klammern sind array
	    					String[] werteTrennung= currentline.split(",");//jede zeile in liste ist jetzt zwischen den werten durch , getrennt
	    					
	    										//[] array hat hier die größe 5
	    					if (werteTrennung.length==5){//bis hierhin:nur wenn zeile 5 werte lang, dann verarbeiten (siehe folgendes)
	    						// Wert1 gibt elementlinie an >> möglichekeiten: neue linie erstellen oder alte verwenden
    ...
    // OES OUTPUT TO EXCEL
    public boolean writeOESFileToXLSXFile(SettingsDataSaver setds, XSSFExcelWriterReader writer, OESFile oesf, XSSFWorkbook wb) throws Exception {
    ...
    private void writeScanToWBColumn(XSSFExcelWriterReader writer, XSSFSheet sheet, OESScan scan, boolean writeTime, int icol) {
    	// Header schreiben
    	int colloff = 0;
		if(writeTime) {
			colloff = 1;
    ...
    /*
    // Alle ElementLinien als WorkSHeet und alle Messungen darein
    public static void outputToExcel(Vector<OESElementLine> elementlinien) { 
    	// Einen Neuen ExcelWriter zum erstellen von .xls Dateien. 
    	// xlsx funktioniert hiermit leider nicht!
        ExcelWriter xlsWriter = new ExcelWriter();
        try {
	        // Ein neues Workbook erstellen
	        xlsWriter.createNewWorkbook("ErstesBuch.xls");

	        // Jede ElementLinie durchgehen
			for (int i=0; i<elementlinien.size(); i++) {
				OESElementLine el = elementlinien.get(i); 
		        // Ein Arbeitsblatt erstellen für jede neue ElementLinie
		        WritableSheet sheet = xlsWriter.getWorksheet(el.Name);
		        
				// Alle Messungen durchgehen
				for(int m = 0; m<el.listMessungen.size(); m++) { 
					OESScan cMessung = el.listMessungen.get(m);
					// Kopfdaten in Excel schreiben
					xlsWriter.addLabel(sheet, m*2, 0, cMessung.name);
					xlsWriter.addLabel(sheet, m*2, 1, cMessung.date);
					xlsWriter.addLabel(sheet, m*2, 2, cMessung.info);
					// Schreibe Daten
					// dafür time und center als kopfzeile
					xlsWriter.addLabel(sheet, m*2, 3, "t [s]");
					xlsWriter.addLabel(sheet, m*2+1, 3, "Intensität");
					// alle Daten einer Messung mit for schleife durchgehen 
					for(int t=0; t<cMessung.time.size(); t++) {
						int row = 4+t;  
						xlsWriter.addLabel(sheet, m*2, row, cMessung.time.get(t));
						xlsWriter.addNumber(sheet, m*2+1, row, cMessung.center.get(t), true);
					}
				} 
			}     
	        
	        // Alles Schreiben und Schließen
	        xlsWriter.closeDataOutputAndSave();
	        // Fertig
        } catch(Exception ex) {
        	ex.printStackTrace();
        } 
...
*/
    ...
    public static void Datenverarbeiten (Vector<OESElementLine> elementlinien, String[] werteTrennung , String kopfzeile){
    	
    	//elementlinie checken (haben wir schon eine), >> möglichekeiten: neue linie erstellen oder alte verwenden
    	//Klasse s.o.
    				//Variable vom Typ (der Klasse) ElementLinie
    	OESElementLine currentElementLinie = null;//currentElementlinie ist neue variable und sie ist leer
    	...
    	//wenn keine elementlinie gefunden, dann neue Elementlinie anlege(in excel später neues worksheet) 
