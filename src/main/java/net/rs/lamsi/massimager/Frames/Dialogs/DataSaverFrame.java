...
public class DataSaverFrame extends JFrame {
  ...
  public DataSaverFrame(Window wnd, SettingsHolder sett) {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        try {
          ...
          if (window.saveDataFile(settings.getSetDataSaver(), excelWriter, currentMode))
            JOptionPane.showMessageDialog(window.getFrame(), "Save of xlsx succeed", "SUCCEED!",
                JOptionPane.CANCEL_OPTION);
          else
            JOptionPane.showMessageDialog(window.getFrame(), "File (.xlsx) not saved", "ERROR",
                JOptionPane.ERROR_MESSAGE);
        } catch (Exception e1) {
          logger.error("",e1);
          // TODO open Message popup
          JOptionPane.showMessageDialog(window.getFrame(), e1.getMessage(), "ERROR",
              JOptionPane.ERROR_MESSAGE);
        }
      }
    });
    ...
    rbAllFiles.addChangeListener(new ChangeListener() {
      public void stateChanged(ChangeEvent e) {
        ...
        try {
          if (rb.isSelected())
            getCbSaveAllFilesInOne().setEnabled(true);
          else
            getCbSaveAllFilesInOne().setEnabled(false);
        } catch (Exception ex) {
        }
      }
    });
  ...
  private void renewAllSettings() {
    ...
    settingsData.setFileFormat(
        (getRbXLS().isSelected() ? SettingsDataSaver.FORMAT_XLS : SettingsDataSaver.FORMAT_TXT));
