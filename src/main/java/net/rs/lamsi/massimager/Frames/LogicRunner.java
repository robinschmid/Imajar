...
public class LogicRunner {
  ...
  public static final int VIEW_TIC = 0, VIEW_MZCHROM = VIEW_TIC + 1, VIEW_SPEC = VIEW_MZCHROM + 1,
      VIEW_MSI_DISCON = VIEW_SPEC + 1, VIEW_MSI_CON = VIEW_MSI_DISCON + 1,
  ...
  public void loadFiles() {
    ...
    /*
     * final File[] files = window.getFilesFromFileChooser(window.getFcOpenMS()); if(files != null)
     * { // Up dateTask ProgressUpdateTask task = ProgressDialog.getInst().startTask(new
     * ProgressUpdateTask(ProgressDialog.getInst(), files.length) { // Load all Files
     * 
     * @Override protected Boolean doInBackground() throws Exception { boolean state = true; // //
     * load file if(files.length>0) { try { // All files in fileList for(File f : files) { // RAW?
  ...
  /*
   * public void loadMzXMLFile(File file) { // TODO Auto-generated method stub // load mzXML with
   * reader if(FileTypeFilter.getExtensionFromFile(file).equalsIgnoreCase("mzXML")) { try {
   * if(reader.openFile(file)) { // List file in fileSpectrumList for Spectrum Information
   * listSpecFiles.add(reader.getSpecList()); // List File in Files List
   * ((DefaultListModel)window.getListFiles().getModel()).addElement(file); } }
   * catch(OutOfMemoryError ex) { logger.error("",ex);
   * JOptionPane.showMessageDialog(window.getFrame(),
   * "Cannot load mzXML-file "+file.getName()+" \n Out of memory! "+ex.getMessage(), "ERROR",
   * JOptionPane.ERROR_MESSAGE); } catch (Exception e) { // TODO Auto-generated catch block
   * logger.error("",e); JOptionPane.showMessageDialog(window.getFrame(),
   * "Cannot load mzXML-file "+file.getName()+" \n"+e.getMessage(), "ERROR",
   * JOptionPane.ERROR_MESSAGE); } } }
   * 
   * private void loadRawFile(File file) { // TODO Auto-generated method stub // Convert via Tool
   * the when Ready load with Reader try {
   * MZFileConverter.convertRAWtoMzXml(window.getSettings().getSetConvertRAW(), this, file); } catch
   * (Exception e) { logger.error("",e); JOptionPane.showMessageDialog(window.getFrame(),
   * "Cannot convert file from .RAW! "+e.getMessage(), "ERROR", JOptionPane.ERROR_MESSAGE); } }
   */
  ...
  public boolean saveDataFile(final SettingsDataSaver settings,
      final XSSFExcelWriterReader excelWriter) throws NoFileSelectedException {
        ...
        protected Boolean doInBackground2() throws Exception {
          ...
          for (int i = 0; i < listWB.length; i++) {
            ...
            if (settings.isExportEIC() && settings.isAllMZInSeperateFiles()) {
              ...
              } else if (i < listSpecFiles.size() && settings.isExportsAllFiles()
                  && !settings.isSavesAllFilesToOneXLS()) {
                // alle in ein eigenes
                retVal = saveDataFileToWB(settings, excelWriter, i, filei, 0);
              } else {
  ...
  // Saving Stuff: With Settings to path, filename, etc
  public boolean saveDataFileALT(SettingsDataSaver settings, XSSFExcelWriterReader excelWriter)
  ...
  private boolean saveDataFileToWB(SettingsDataSaver settings, XSSFExcelWriterReader xlsWriter,
      int listWBIndex, int specFileIndex, int filesExported) {
    ...
    try {
      ...
      if (settings.isExportTIC()) {
        ...
        // xlsWriter.writeToCell(sheet, 1+colloffset, 2,
        // spec.getHeader()).setCellStyle(borderStyle);
        // data header
        xlsWriter.writeToCell(sheet, 1 + colloffset, 3, "intensity (cps)")
            .setCellStyle(borderStyle);
        // TICDATA
        writeMZChromToXLSAtPosition(xlsWriter, MZDataFactory.getTIC(spec), wb, sheet,
            0 + colloffset, 4, onlyY);
      }
      ...
      if (settings.isExportSpectrum()) {
        ...
        xlsWriter.writeToCell(sheet, 1 + normcolloffset, 0, spec.getName())
            .setCellStyle(borderStyle);
        // xlsWriter.writeToCell(sheet, 1+normcolloffset, 1,
        // spec.getFile().getPath()).setCellStyle(borderStyle);
        // xlsWriter.writeToCell(sheet, 1+normcolloffset, 2,
        // spec.getHeader()).setCellStyle(borderStyle);
        // get RT for spectrum
        double rt = 0;
        try {
        ...
        xlsWriter.writeToCell(sheet, 1 + normcolloffset, 4, "intensity (cps)")
            .setCellStyle(borderStyle);
        // TICDATA
        writeMZChromToXLSAtPosition(xlsWriter, MZDataFactory.getSpectrumAsMZChrom(spec, rt), wb,
            sheet, 0 + normcolloffset, 5, false);
      }
      ...
      if (settings.isExportEIC()) {
        ...
        for (int i = 0; i < mzList.size(); i++) {
          ...
          // TimeRelated immer
          xlsWriter.writeToCell(sheet, 1 + colloffset, 0, spec.getName()).setCellStyle(borderStyle);
          // xlsWriter.writeToCell(sheet, 1+colloffset, 1,
          // spec.getFile().getPath()).setCellStyle(borderStyle);
          // xlsWriter.writeToCell(sheet, 1+colloffset, 2,
          // spec.getHeader()).setCellStyle(borderStyle);
          // data header
          xlsWriter.writeToCell(sheet, 1 + colloffset, 3, "intensity (cps)")
              .setCellStyle(borderStyle);
          // TICDATA
          writeMZChromToXLSAtPosition(xlsWriter,
              MZDataFactory.getMZChrom(spec, mz, ChromGenType.HIGHEST_PEAK), wb, sheet,
              0 + colloffset, 4, onlyY);
        }
      }
  ...
  private boolean saveDataFileWithSeperatedMZ(SettingsDataSaver settings,
      XSSFExcelWriterReader xlsWriter, int listWBIndex, int mzExported, MZIon mz) {
    // in ein file oder mehrere
    try {
      // Get WorkBook:
      // WB
  ...
  // MZChrom to xls
  private void writeMZChromToXLSAtPosition(XSSFExcelWriterReader xlsWriter, MZChromatogram chrom,
      XSSFWorkbook wb, XSSFSheet sheet, int scol, int srow, boolean onlyY) {
    // style
    CellStyle borderStyle = wb.createCellStyle();
    borderStyle.setBorderRight(CellStyle.BORDER_MEDIUM);
    ...
    for (int i = 0; i < chrom.getItemCount(); i++) {
      // x (time or m/z)
      if (!onlyY)
        xlsWriter.writeToCell(sheet, scol, srow + i, chrom.getX(i).doubleValue());
      // y (intensity)
      xlsWriter.writeToCell(sheet, scol + 1, srow + i, chrom.getY(i).doubleValue())
          .setCellStyle(borderStyle);
    }
  }
  ...
  /*
   * TODO returns spectrum getSpec(rt) DONE getSpec(rt0,rt1) DONE getSpec(Settings x,y) DONE
   * getSpec(x,y,x2,y2 DONE returns chartpanel getChartPanelFromSpec(Spectrum getTIC getEIC(mz,pm,
   * getImage(SettingsImage , mz, pm - discon / con is defined by settingstype --> check class
   */
  ...
  public MZChromatogram generateSpectrumSUMByRT(double rt0, double rt1) {
    try {
      if (selectedFileIndex != -1)
        return MZDataFactory.getSpectrumSumAsMZChrom(listSpecFiles.get(selectedFileIndex), rt0,
            rt1);
      else
        return null;
    } catch (Exception ex) {
    ...
    try {
      ...
      // raw is retention time data
        ...
        if (sett.isTriggered()) {
          ...
          if (filei < 0)
            filei = 0;
          if (filei >= listSpecFiles.size())
            filei = listSpecFiles.size() - 1;
          // rt by x
  ...
  public MZChromatogram generateSpectrumByXY(SettingsGeneralImage sett, float x, float y, float x2,
    ...
    try {
      ...
      if (getSelectedFile() instanceof ImagingRawData) {
        ...
        ArrayList<Scan> list = raw.getScans(x / sett.getVelocity(), y / sett.getSpotsize(),
            x2 / sett.getVelocity(), y2 / sett.getSpotsize());
      ...
      // raw is retention time data
        ...
        if (sett.isTriggered()) {
          ...
          if (fileiStart < 0)
            fileiStart = 0;
          if (fileiStart >= listSpecFiles.size())
            fileiStart = listSpecFiles.size() - 1;
          // End file index
          fileiEnd = (int) Math.floor(y2 / sett.getSpotsize());
          if (fileiEnd < 0)
            fileiEnd = 0;
          if (fileiEnd >= listSpecFiles.size())
            fileiEnd = listSpecFiles.size() - 1;
          // rt by x
          rt = x / sett.getVelocity();
  ...
  public ChartPanel generateTICAsChartPanel() throws Exception {
    if (selectedFileIndex != -1 && listSpecFiles != null)
      return MZDataFactory.getTIC(listSpecFiles.get(selectedFileIndex)).getChromChartPanel("TIC",
          "time", "intensity");
    else
      return null;
  }
  ...
  public ChartPanel generateEICAsChartPanel(MZIon mzIon) throws Exception {
    if (selectedFileIndex != -1 && listSpecFiles != null)
      return MZDataFactory
          .getMZChrom(listSpecFiles.get(selectedFileIndex), mzIon, ChromGenType.HIGHEST_PEAK)
          .getChromChartPanel("MZ = " + mzIon.getMz(), "time", "intensity");
    else
      return null;
  }
  ...
  public Image2D generateImageDiscon(SettingsMSImage setMSIDiscon) {
    ...
    if (specList != null) {
      ...
      for (int i = 0; i < specList.size(); i++) {
        mzChrom[i] = MZDataFactory.getMZChrom(specList.get(i), setMSIDiscon.getMZIon(),
            ChromGenType.SUM_PEAKS);
      }
      // set title
      ...
      //
      Image2D img = Image2D.generateImage2DFrom(SettingsPaintScale.createStandardSettings(),
          setMSIDiscon, mzChrom);
  ...
  public RawDataFile getSelectedFile() {
    return selectedFileIndex >= 0 && selectedFileIndex < listSpecFiles.size()
        ? listSpecFiles.get(selectedFileIndex)
        : null;
  }
