...
public class ChargeCalculatorSettingsDialog extends JDialog {
	...
	public ChargeCalculatorSettingsDialog() {
		...
		}
		{
			...
			comboAbsRT.setModel(new DefaultComboBoxModel(new String[] {"absolute (min)", "relative (%)"}));
	...
	protected void saveToChargeCalcSettings() {
		...
		try {
			sett.setMzTolerance(new MZTolerance(Double.valueOf(getTxtMZTolerance().getText()), Double.valueOf(getTxtMZTolerancePPM().getText())));
