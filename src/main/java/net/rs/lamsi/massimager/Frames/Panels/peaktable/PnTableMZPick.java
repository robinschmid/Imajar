...
public abstract class PnTableMZPick extends JPanel implements ListSelectionListener {
  ...
  protected String[] columnToolTips = {"ID", "Rawdata file", "Used PeakList",
      "m/z (mass per charge)", "Calculated charge (can be modified)",
      "Mass calculated by mz*charge", "Average height of this peak", "Average area of this peak",
  ...
  public PnTableMZPick() {
    ...
    tableMZPeak = new JTable() {
      ...
      public TableCellRenderer getCellRenderer(int row, int column) {
        SettingsGeneralValueFormatting settings =
            Window.getWindow().getSettings().getSetGeneralValueFormatting();
        ...
        switch (commonColumn) {
          ...
          case AREA:
          case HEIGHT:
            renderer.setFormat(settings.getDecimalsIntensity(),
                settings.isShowingExponentIntensity());
            return renderer;
        }
        // else...
  ...
  public void addPeak(PeakListRow peak, PeakList peakListName, double rtMin, double rtMax) {
    //
    String rawName = "";
    if (peakListName != null && peakListName.getRawDataFiles().length == 1)
      rawName = peakListName.getRawDataFile(0).getName();
    PeakTableRow row =
        new PeakTableRow("#" + model.getRowCount(), rawName, peakListName, peak, rtMin, rtMax, 0);
    model.addRow(row);
    //
