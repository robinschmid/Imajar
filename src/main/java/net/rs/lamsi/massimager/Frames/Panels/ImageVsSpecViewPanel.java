...
/*TODO
 * WICHTIG:
 * das fenster das ausgewählt ist in das kann rein gezoomt werden.
 * wenn in andere Views geklickt wird, dann wird rt/mz für das ausgewählte view selektiert!
 * wenn STRG gehalten wird, dann kann in jedes rein gezoomt werden
 */

public class ImageVsSpecViewPanel extends JPanel implements Runnable {
	// MODES
	// TODO image als 3d EIC
	public static final String VK_LEFT = "VK_LEFT", VK_RIGHT = "VK_RIGHT", VK_UP = "VK_UP", VK_DOWN = "VK_DOWN",
			VK_DELETE = "VK_DELETE", VK_F5 = "VK_F5", VK_1 = "VK_1", VK_2 = "VK_2", VK_3 = "VK_3";
	public static final String VK_LEFT_RELEASED = "VK_LEFT_RELEASED", VK_RIGHT_RELEASED = "VK_RIGHT_RELEASED", VK_UP_RELEASED = "VK_UP_RELEASED", 
			VK_DOWN_RELEASED = "VK_DOWN_RELEASED", VK_DELETE_RELEASED = "VK_DELETE_RELEASED", VK_F5_RELEASED = "VK_F5_RELEASED", 
	...
	public ImageVsSpecViewPanel(Window wnd) {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if(!e.getValueIsAdjusting()) {
					...
					if(row!=-1) { 
						...
						Feature[] peak = tableMzPeak.getTableModel().getPeakRowList().get(row).getPeakRow().getPeaks();
		...
		btnMiddleTIC.setSelectedIcon(new ImageIcon(ImageVsSpecViewPanel.class.getResource("/img/btn_tic_selected.png")));
		...
		btnMiddleEIC.setSelectedIcon(new ImageIcon(ImageVsSpecViewPanel.class.getResource("/img/btn_mz_selected.png")));
		...
		btnMiddleImageDisc.setSelectedIcon(new ImageIcon(ImageVsSpecViewPanel.class.getResource("/img/btn_img_D_selec.png")));
		btnMiddleImageDisc.setIcon(new ImageIcon(ImageVsSpecViewPanel.class.getResource("/img/btn_img_D.png")));
		...
		btnMiddleImageCon.setIcon(new ImageIcon(ImageVsSpecViewPanel.class.getResource("/img/btn_img_C.png")));
		btnMiddleImageCon.setSelectedIcon(new ImageIcon(ImageVsSpecViewPanel.class.getResource("/img/btn_img_C_selec.png")));
		...
		tglbtnOri.setSelectedIcon(new ImageIcon(ImageVsSpecViewPanel.class.getResource("/img/btn_split_change_selec.png")));
		tglbtnOri.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				JToggleButton btn = (JToggleButton)e.getSource(); 
				getSplitMZvsSpec().setOrientation(btn.isSelected()? JSplitPane.HORIZONTAL_SPLIT: JSplitPane.VERTICAL_SPLIT);
			}
		});
	...
	public void setIsShown() {
		...
		if(selectedSpectrum==null) {
			MZChromatogram spec = MZDataFactory.getSpectrumAsMZChrom(window.getLogicRunner().generateSpectrumByRT(0));
			if(spec!=null) renewBottomSpectrum(spec); 
		} 
		//
		...
		try {
			...
			if(selectedModeMiddle==MODE_TIC)
				...
				if(selectedModeMiddle==MODE_EIC)
					chart = window.getLogicRunner().generateEICAsChartPanel(new MZIon("", selectedVsMiddleMZ, selectedVsMiddlePM));
				...
				if(e.getButton()==MouseEvent.BUTTON1) {
					...
					if(pressed!=null) {
						if(released.getY()>=yrange.getLowerBound() && released.getY()<=yrange.getUpperBound() &&
								released.getX()>=xrange.getLowerBound() && released.getX()<=xrange.getUpperBound()) {
							// dann setzen 
	...
	protected ChartPanel getCurrentChartMiddle() {
		...
		// also neue im middle
		if(selectedModeMiddle==MODE_EIC || selectedModeMiddle==MODE_TIC) return chartMiddleChrom;
		else return chartMiddleImage; 
	}
	...
	protected void addMZtoTable(double mz, double pm) { 
		...
		if(selectedPeakList!=null) {
			...
			if(getMenuTopTableActions().getBtnOnlyHighestPeak().isSelected()) { 
				...
				PeakListRow highestRow = getHighestPeak(rows, selectedPeakList, selectedVsRetentionTime[0], selectedVsRetentionTime[1]);
			...
			Window.getWindow().showErrorDialog("No peaklist selected", "Select a MZMine2-PeakList. To generate one go to MZMine: \n 1. Mass detection\n 2. Chromatogram builder \n 3. Chromatogram deconvolution");
		}
	}
	/**
	 ...
	 */
	public PeakListRow getHighestPeak(PeakListRow[] rows, PeakList peakListName, double rtMin, double rtMax) { 
		// save highest peak
	...
	public void setSelectedView(int selectedView) {
		...
		if(chartBottomSpec!=null) chartBottomSpec.setMouseZoomable(selectedView==VIEW_BOTTOM_SPECTRUM);
		if(chartMiddleChrom!=null) chartMiddleChrom.setMouseZoomable(selectedView==VIEW_MIDDLE_IMAGECHROM);
	...
	//##########################################################################################
	/* KeyListener:
	 *  a.	DELETE: delete row in peaktable
		b.	ARROWS: move selected ChartPanel
		c.	1/2/3: select chartView
		d.	F5: update charts (image update ..) 
	 */ 
	...
	protected long keyTimeArrowLeft = -1, keyTimeArrowDown = -1, keyTimeArrowRight = -1, keyTimeArrowUp = -1;
