...
//	private static Vector<MZMineRawDataListsChangedListener> rawDataListListeners = new Vector<MZMineRawDataListsChangedListener>();
	...
	private static ArrayList<MZmineProjectListener> projectListener = new ArrayList<MZmineProjectListener>();
		...
		parentProjectListener = new MZmineProjectListenerAdapter() {
			...
			public void peakListsChanged(PeakList pkl, Operation op) {
				for(MZmineProjectListener l : projectListener)
					if(op.equals(Operation.ADDED)) l.peakListAdded(pkl);
					else l.peakListRemoved(pkl);
			}
			
			@Override
			public void dataFilesChanged(RawDataFile raw, Operation op) {
				for(MZmineProjectListener l : projectListener)
					if(op.equals(Operation.ADDED)) l.dataFileAdded(raw);
					else l.dataFileRemoved(raw);
			}
		};
...
//		mainPN.getRawDataTree().getModel().addTreeModelListener(new RawDataListChangedListener());
	...
	public static void callAllListenersForRawAndPeakLists() { 
		parentProjectListener.dataFilesChanged(null, null);
		parentProjectListener.peakListsChanged(null, null);
	}
	// Called by Project change to load all
