...
public class MZChargeCalculatorDouble {
   ...
   * @param parameters
   */
  public MZChargeCalculatorDouble(Vector<Point2D> peakList, SettingsChargeCalculator parameters,
      Point2D peakMZ) {
    ...
    mzTolerance = new MZTolerance(parameters.getMzTolerance().getMzTolerance() * 3,
        parameters.getMzTolerance().getPpmTolerance() * 2);
  ...
  public int doFiltering() {
    ...
    if (bestFitScore < 3)
      bestFitCharge = 0;
    // setting the charge
    System.out.println("Charge = " + bestFitCharge + "    bestFItScore = " + bestFitScore
   ...
   */
  private void fitPattern(Vector<Point2D> fittedPeaks, Point2D p, int charge,
      Vector<Point2D> sortedPeaks) {
   ...
   */
  private void fitHalfPattern(Point2D p, int charge, int direction, Vector<Point2D> fittedPeaks,
      Vector<Point2D> sortedPeaks) {
    ...
    do {
      ...
      for (int ind = 0; ind < sortedPeaks.size(); ind++) {
        ...
        if (mzTolerance.checkWithinTolerance(isotopeMZ, mainMZ)
            && (!fittedPeaks.contains(candidatePeakMZ))) {
